      ...
      >
        <div className="flex justify-between">
          <h1 className="mt-6 mb-4 ml-2 text-xl font-semibold">
            <Link to="/">{t('home-page.title')}</Link>
          </h1>
          <button
            className="flex h-8 w-8 items-center justify-center self-center rounded-full bg-gray-100 hover:bg-gray-200"
            onClick={() => {
        ...
        {filters.filterCounter > 0 && (
          <div className="flex justify-between px-5 pb-4">
            <span className="flex items-center text-sm font-bold text-blue-600">
              {otherCharacters.length + starredCharacters.length || 0}{' '}
              {t('home-page.results-filter')}
            </span>
            <span className="bg-secondary-600/20 text-secondary-700 rounded-full px-3 py-0.5">
              {filters.filterCounter} {t('home-page.applied-filters')}
            </span>
          </div>
        )}
        ...
        {!loading && starredCharacters.length > 0 && (
          <div className="mb-4">
            <h2 className="mb-2 ml-5 text-sm text-gray-400 uppercase">
              {t('home-page.starred-characters')} ({starredCharacters.length})
            </h2>
        ...
        {!loading && otherCharacters.length > 0 && (
          <>
            <h2 className="mb-2 ml-5 text-sm text-gray-400 uppercase">
              {t('home-page.other-characters')} ({otherCharacters.length || 0})
            </h2>
            <ul className="divide-y divide-gray-200">
        ...
        <main className="hidden flex-1 flex-col bg-white px-6 py-6 md:flex">
          <div className="flex h-screen flex-1 items-center justify-center">
            <p className="text-lg text-gray-500">
              {t('home-page.character-not-selected')}
            </p>
          </div>
        </main>
      )}
